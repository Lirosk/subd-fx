package com.example.subd.Helpers;

public class Urls {
    public static final String SIGNUP = "signup-view.fxml";
    public static final String LOGIN = "login-view.fxml";
    public static final String USERMAINMENU = "usermainmenu-view.fxml";
    public static final String SUBSCRIPTIONS = "subcriptions-view.fxml";
    public static final String BOOKS = "books-view.fxml";
    public static final String PROMOTIONS = "promotions-view.fxml";
    public static final String FUTUREGOALS = "futuregoals-view.fxml";

    public static final String TIME_TO_CHOOSE_WHO_YOU_ARE = "time_to_choose_who_you_are-view.fxml";

    public static final String EMPLOYEEMAINMENU = "employeemainmenu-view.fxml";
    public static final String USERS = "users-view.fxml";
    public static final String BRANCHEMPLOYEES = "mybranchemployees-view.fxml";
    public static final String MYFUTUREGOALS = "myfuturegoals-view.fxml";
    public static final String LOGS = "logs-view.fxml";
}
